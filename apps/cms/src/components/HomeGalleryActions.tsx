...
export function HomeGalleryActions() {
  ...
  async function refresh() {
    ...
    if (!response.ok) throw new Error("暂时无法读取首页照片状态。");
    const result = await response.json();
    setState(result.homeGallery);
  }
  ...
  useEffect(() => {
    ...
    } else void refresh().catch(() => setError("暂时无法读取首页照片状态。"));
  }, [modified]);
  ...
  async function run(action: "preview" | "publish") {
    ...
    if (
      action === "publish" &&
      !window.confirm("发布刚才预览的首页照片与展示样式到官网？")
    )
      return;
    setBusy(true);
    setMessage("");
  ...
  return (
    <section className="content-document-actions" aria-label="首页照片操作">
      ...
      <p>先保存下方照片和样式，再生成预览；确认效果后发布该预览版本。</p>
