...
目标 ECS 为 106.15.44.81，域名 empact.cn。2026-09-20 已通过维护人确认的 `root@106.15.44.81` 连接服务器，并安装官网独立服务。现有 Caddy 位于 `chatcircle-caddy-1` 容器，配置源文件为 `/opt/chatcircle/deploy/Caddyfile`；修改前须备份、验证，再 reload。实际发布版本及域名验收见 [首次服务器部署记录](deployment-2026-09-20.md)。SSH 用户和密钥仍应以维护人确认的连接方式为准。
