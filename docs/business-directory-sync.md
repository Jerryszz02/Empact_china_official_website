...
依据 2026-09-19 工作区的 `Empact网站业务目录.xlsx`，工作表为「业务目录」。仅把表格中的业务名称、案例、链接和附件作为内容来源；负责人提醒、协作备注及空案例位不进入网站。原工作簿和既有素材整理保持原样。
...
已通过 `agent-browser` 读取推文正文，逐项核对标题与活动内容。朝夕有爱推文正文为长图，另行查看原图确认了长者陪伴、防跌科普与安装防滑垫的内容。网站使用 `detailUrl` 直接跳转，不复制推文正文。
...
沿用已核对的 Office Camp、香港研学、少年说和朝夕有爱素材。缺少已核实活动图的卡片使用明确标注的 Empact 品牌标识，不把其他活动照片当作现场证据。媒体复制到 `apps/site/public/media/directory-*`，原素材不改动。
...
| D15             | AI×思辨线上课程                  | 表格未附专用课程材料；现有 AI 系统课大纲不能自动视为这一版线上课程。需课程简介、大纲或原推文                                                      |
