...
后台：[http://127.0.0.1:3000/admin](http://127.0.0.1:3000/admin)。本机初始化命令将随机账号凭据保存到 **`.data/local-admin.json`**（权限 600），不打印密码；已有账号和内容不会覆盖。`.env`、数据库、媒体与预览均被 Git 排除。不能把本机草稿部署到公开预览地址。

若要在本机演练审批后的发布，再开一个终端运行 `npm run serve`，在 127.0.0.1:4322 接收并核验已发布版本；4321 始终是结构预览。未审批资料仍无法正式发布。
